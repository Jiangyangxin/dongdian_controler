...
void FeatureTracker::setMask()
{
    if(FISHEYE)
        ...
        mask = cv::Mat(ROW, COL, CV_8UC1, cv::Scalar(255));//默认的mask是一个跟相机相同分辨率的白色矩形
...
void FeatureTracker::readColor(const cv::Mat &_img, double _cur_time)
{
    cv::Mat img;
    TicToc t_r;
    cur_time = _cur_time;
