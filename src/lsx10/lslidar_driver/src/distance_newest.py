...
class Lidar_Bladelenth():
    def __init__(self,publisher) -> None:
        ...
        self.left_right_ybias=0
    
    def find_sequence(self,arr, seq):#在arr中查找序列seq，每个序列都由0隔开，序列由0结尾
        seq = np.array(seq)
        arr = np.array(arr)
        n = seq.size
        count=0
    ...
    def is_top_much_error(self,Bboundary,nowBboundary):#arr1作为相对误差的基准 Bboundary,nowBboundary
        temp_x1=-Bboundary[0]
        temp_y1=-Bboundary[1]
        temp_x2=-Bboundary[2]
        temp_y2=-Bboundary[3]

        temp_x1_now=-nowBboundary[0]
        temp_y1_now=-nowBboundary[1]
        temp_x2_now=-nowBboundary[2]
        temp_y2_now=-nowBboundary[3]      
        
        error=abs(temp_y1-temp_y1_now)
...
###### 全局变量的值赋予局部变量，方便修改整体代码
        Bdis = self.Bdis
        Bboundary  = self.Bboundary
        Bangle= self.Bangle
        count = self.count
        times= self.times
        ...
        if count==0.: #初始化
            ...
            count=1.
            str=" "
        else:#初始化结束，根据本次迭代的最大物体的长度，边界位置进行不同操作。
            ...
            if abs(Blen-nowBlen)<tolerance*Blen and self.each_relative_error(Bboundary,nowBboundary):#如果长度差值和边界坐标的差值在限定范围内，
                ...
                str="similar"     
            elif abs(Blen-nowBlen)<tolerance*Blen and not self.each_relative_error(Bboundary,nowBboundary): #长度差值在限定范围内，但是边界坐标差值超过限度
                ...
                if Blen>nowBlen:
                    ...
                    Bdis=nowBdis.copy()
                    count = 1.
            elif abs(nowBlen-Blen)>tolerance*Blen and not self.is_top_much_error(Bboundary,nowBboundary): #如果长度差值过大，但边界坐标的差值在限定范围内
                #此时大概率是工装移动了
                if nowBlen>Blen: 
                    # print("目前长度")
                    # print(nowBlen)
                    ...
                    str = "bigger"
                # else:#更小物体
                elif nowBlen<Blen and self.lidarinit<50: #短时间内发生nowBlen<Blen
                    self.lidarinit+=1
                    ...
                    Bboundary=nowBboundary
                    count=1.
                    print(("/***第%u次迭代,监测到工装移动，边界调整***/") % times) 
                    str = "shorter"
                    # print(("/***第%u次迭代,检测到更小物体,长度为%g m***/") %(times,nowBlen))    
            elif abs(nowBlen-Blen)>tolerance*Blen and self.is_top_much_error(Bboundary,nowBboundary): #如果长度差值过大，且边界坐标的差值不在限定范围内  
                #说明此时大概率是机器人进入了雷达视野
                if nowBlen>Blen: 
                    self.lidarinit=0
                    Bdis=nowBdis.copy()
                    ...
                    print(("/***第%u次迭代,监测到机器人进入雷达，边界不调整***/") % times) 
        ...
        for i in range(4):
            self.lidar_info_array.data.append(Bboundary[i])
            # self.lidar_info_array.data.append(nowBboundary[i])
        self.lidar_info_array.data.append(float(self.middle_dis)) #nowBdis 保存着最大物体扫描出的每个点的距离数据
        ...
        self.lidar_info_array.data[0]= -temp_y1     #new_x1 = -old_y1
        self.lidar_info_array.data[1]=  -temp_x1+self.left_right_ybias     #new_y1 = -old_x1
        self.lidar_info_array.data[2]= -temp_y2     #new_x2 = -old_y2
        self.lidar_info_array.data[3]=  -temp_x2+self.left_right_ybias      #new_y2 = -old_x2
...
###### 局部变量恢复为全局变量
    ...
    def neighbor_diff(self,dis,tolerance):#检测相邻有效点的差值是否超过测量误差，并进一步去除无效点
    ...
    def callback(self,msg):
        ...
        angle_range=np.array([225,325],dtype=float)
        ...
        # print("rawdistance:",rawdistance.size)
        try:
            if not np.isinf(rawdistance[120]) and rawdistance[120] > 0: #120 为雷达90度方向的值
                self.middle_dis=rawdistance[120]
        except Exception as e:
            print("media_lidar error:",e)         
        if rawdistance[-1]!=0:
            rawdistance[-1]=0.
        distance= np.where(rawdistance > 8,0, rawdistance)#无效距离赋0
        
        tolerance_dis = 0.8  # 雷达测量误差是 ±3cm.机器人高度24cm
        distance = self.neighbor_diff(distance,tolerance_dis)  # 处理测量误差过大的点
                ...
                if validpts_count>1:  #无效点之前是有效点，且有一个以上有效点
                    ...
                    if self.getlength(validpts_dis,angle_incre)<obj_minlen or self.getlength(validpts_dis,angle_incre)>obj_maxlen:#忽略长度不在限度内的物体
                        ...
                        if (i + invalidpts_count)<=distance.size-1:#如果这组无效点之后的点存在
                            if invalidpts_count <= 3 and abs(distance[i - 1] - distance[i + invalidpts_count])<tolerance_dis:
                                temp=validpts_count
                                validpts_count = 0
                                continue
...
def my_function1():
    ...
    lidar_left.left_right_ybias=0.30
    rospy.Subscriber("/LeftLidar/scan_left", LaserScan, lidar_left.callback, queue_size=1)
...
def my_function2():
    ...
    lidar_right.left_right_ybias=0
    rospy.Subscriber("/RightLidar/scan_right", LaserScan, lidar_right.callback, queue_size=1)
    # while not rospy.is_shutdown():                
    #     rospy.spin() 
    # if(rospy.is_shutdown()):
    #     time.sleep(1.0)
...
###jyx最新修改，必须打开两个ros节点才能正常使用，即 roslaunch lslidar_driver distance.launch

def listener():
    # global lidar_info_pub
    rospy.init_node('lasr_listener', anonymous=True)
    #创建多线程
    lidar_base = rospy.get_param("~lidar_base", "left")

    if lidar_base=="left" :
        lidar_left_thread = threading.Thread(target=my_function1)
        lidar_left_thread.start()

    elif lidar_base=="right" :
        lidar_right_thread = threading.Thread(target=my_function2)
        lidar_right_thread.start()        
