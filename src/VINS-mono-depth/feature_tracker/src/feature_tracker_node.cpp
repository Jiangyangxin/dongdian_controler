        ...
        for (int i = 0; i < NUM_OF_CAM; i++)
        {
            ...
            for (unsigned int j = 0; j < ids.size(); j++)
            {
                if (trackerData[i].track_cnt[j] > 1)
                {
                    ...
                    //depth_of_point.values.push_back(150.0);
                    ROS_INFO("depth measurment: %f",depth_of_point.values.back());
...
{
    //ROS_INFO("get color measurement!");
    if(first_image_flag)
    {
        first_image_flag = false;
        first_image_time = color_msg->header.stamp.toSec();
    ...
    //cv_bridge::CvImageConstPtr ptr;//jyx：此处为源代码，为了选取ROI区域，将const删除
    ...
    // cv::Mat ROI_img= ptr->image(roiRect);
    // ptr->image=ROI_img;
    // ROW=height;
    // COL=width;
   ///
    ...
    {
        ROS_DEBUG("processing camera %d", i);
        if (i != 1 || !STEREO_TRACK)//单目执行这个
        {

            trackerData[i].readColor(ptr->image.rowRange(ROW * i, ROW * (i + 1)), color_msg->header.stamp.toSec());
        }
        else
        {
            if (EQUALIZE)
        ...
        for (int i = 0; i < NUM_OF_CAM; i++)
        {
            ...
            for (unsigned int j = 0; j < ids.size(); j++)
            {
                if (trackerData[i].track_cnt[j] > 1)
                {
                    ...
                    // show_depth: 480*640   y:[0,480]   x:[0,640]
                    //depth_of_point.values.push_back((int)show_depth.at<unsigned short>(round(cur_pts[j].y), round(cur_pts[j].x)));
                    depth_of_point.values.push_back(FAKE_DEPTH);
                    //ROS_INFO("depth %f",depth_of_point.values.back());
                    //debug use: print depth pixels
                    //test.push_back((int)show_depth.at<unsigned short>(round(cur_pts[j].y),round(cur_pts[j].x)));
                }
            }
...
int main(int argc, char **argv)
{
    ...
    if(FAKE_DEPTH) //若调用虚假的深度信息，执行color_callback
    {
