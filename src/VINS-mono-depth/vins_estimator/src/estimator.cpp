...
Estimator::Estimator(): f_manager{Rs}
{
    ...
    ROS_INFO("Static initialization flag %d",static_init_flag);
    clearState();
}
...
void Estimator::setParameter()
{
    ...
    td = TD;
    static_init_flag = STATIC_INIT_FLAG;
    std::cout<<"static_init_flag:"<<static_init_flag<<std::endl;
}
...
void Estimator::processIMU(double dt, const Vector3d &linear_acceleration, const Vector3d &angular_velocity)
{
    ...
    gyr_0 = angular_velocity;
    // by gzf
    imu_init.imu_push(dt, linear_acceleration, angular_velocity);
}
...
void Estimator::processImage(const map<int, vector<pair<int, Eigen::Matrix<double, 8, 1>>>> &image, const std_msgs::Header &header)
{
    ...
    // by gzf(attention!!):由于机器人静止时，视差是不变的，因此采取原来视差决定的关键帧策略会导致second new frame 与third new frame之间的时间间隔巨大
    // 在IMU参数初始化不好的时候，会导致机器人在second new frame开始有较大的漂移(无法及时校准imu参数引起的)
    double time_now = header.stamp.toSec();
    if (f_manager.addFeatureCheckParallax(frame_count, image, td) || time_now - second_new_time > 0.5)
    {
        marginalization_flag = MARGIN_OLD;
        second_new_time = time_now;
    }
    else
...
bool Estimator::initialStructure()
{
    ...
    {
        if (static_init_flag)
            ROS_INFO("Using static initial; Keep device still");
        else
...
bool Estimator::relativePose(Matrix3d &relative_R, Vector3d &relative_T, int &l)
{
    ...
    for (int i = 0; i < WINDOW_SIZE; i++)
    {
        ...
        if (corres.size() > 20)
        {
            ...
            // by gzf 静态初始化要求视差为0，即机器人保持静止；动态初始化需要有运动激励
            // if (static_init_flag)
            if (!static_init_flag)
            {
            ...
            {
                if (average_parallax * 460 < 1 && m_estimator.solveRelativeRT_PNP(corres, relative_R, relative_T))
                {
                    //                Matrix3d relative_R2; Vector3d relative_T2;
                    //                m_estimator.solveRelativeRT(corres, relative_R2, relative_T2);
                    l = i;
...
bool Estimator::failureDetection()
{
    ...
    if (Bas[WINDOW_SIZE].norm() > 25)//2.5
    {
...
/*静态初始化部分*/
bool Estimator::static_visualInitialAlign()
{
    VectorXd x;
    // solve scale
    // 1.计算陀螺仪偏置，尺度，重力加速度和速度
    // bool result = VisualIMUAlignment(all_image_frame, Bgs, g, x);
    bool result = imu_init.imu_init(all_image_frame, Bgs, g, x);
    if (!result)
    {
    ...
    for (int i = frame_count; i >= 0; i--)
    {
        // ROS_INFO("Ps[i] before : %f %f %f %f",Ps[i](0),Ps[i](1),Ps[i](2),s);
        Ps[i] = s * Ps[i] - Rs[i] * TIC[0] - (s * Ps[0] - Rs[0] * TIC[0]);
        // ROS_INFO("Ps[i] after : %f %f %f",Ps[i](0),Ps[i](1),Ps[i](2));
    }
    int kv = -1;
    map<double, ImageFrame>::iterator frame_i;
    for (frame_i = all_image_frame.begin(); frame_i != all_image_frame.end(); frame_i++)
    ...
    // 计算将g转到{0,0,1}方向的旋转矩阵，由于yaw有一个自由度，函数内已补偿，默认yaw为0. ypr*g;
    Matrix3d R0 = Utility::g2R(g);
    // 计算相机根据R0，将重力与z轴对齐后，相机的初始yaw角
    double yaw = Utility::R2ypr(R0 * Rs[0]).x();
    R0 = Utility::ypr2R(Eigen::Vector3d{-yaw, 0, 0}) * R0;
    // 将重力调整到z轴上
    // 疑问：为什么调整后 g的值为（0,0,9.81）而不是（0,0,-9.81）
    g = R0 * g;
    // Matrix3d rot_diff = R0 * Rs[0].transpose();
    Matrix3d rot_diff = R0;
