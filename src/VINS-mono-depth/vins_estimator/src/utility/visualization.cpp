...
void pubTF(const Estimator &estimator, const std_msgs::Header &header , const std::string &tf_prefix)
{
    if( estimator.solver_flag != Estimator::SolverFlag::NON_LINEAR)
        return;
    ...
    // br.sendTransform(tf::StampedTransform(transform, header.stamp, tf_prefix+"world", tf_prefix+"body"));
    ...
    transform.setRotation(q);
    // br.sendTransform(tf::StampedTransform(transform, header.stamp, tf_prefix+"body", tf_prefix+"camera"));
